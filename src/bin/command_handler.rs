...
async fn read_all_events(client: Client) -> Result<(), Box<dyn Error>>  {
    ...
    let filter = SubscriptionFilter::on_event_type().add_prefix("stockItem").exclude_system_events();
    ...
    loop {
        ...
        println!("Received event {}@{}, {:?}", revision, stream_id, event.get_original_event());
    }
}